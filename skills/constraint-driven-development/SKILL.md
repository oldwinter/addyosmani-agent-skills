...
这是 `constraint-driven-development` 的中文 runtime 入口。

中文工程请求命中本 skill 时，先判断项目是否已有 `CONSTRAINTS.md`；已有且本次不改质量标准时先读取并遵守，缺失时按上游流程建立项目级质量契约。交互场景最多逐项询问四个问题；CI、`/loop` 或其他自主运行场景不询问用户，直接采用 Floor 并标注其余约束待人工确认。每个带数字的约束都必须绑定真实可运行的检查命令并标明执行阶段；不要通过新增 suppression、跳过或删除测试、移除断言、加入未完成 stub、降低阈值或新增无期限例外来让检查变绿。输出说明使用简体中文；保留命令、参数、路径、URL、代码、schema、测试精确字符串和 skill slug。
